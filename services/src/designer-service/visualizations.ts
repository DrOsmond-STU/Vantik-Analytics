...
export interface VisualDefinition {
  code: string;
  /** Nama visual tidak diterjemahkan — istilah teknis lazim bilingual (DESIGN.md 8.1). */
  label: string;
  family: VisualFamily;
  minMeasures: number;
 ...
 */
export const VISUALIZATION_CATALOG: readonly VisualDefinition[] = [
  // --- Trend ---
  v('line', 'Line', 'trend'),
...
export const VISUALIZATION_COUNT = VISUALIZATION_CATALOG.length;

export function visualsByFamily(): Map<VisualFamily, VisualDefinition[]> {
  const out = new Map<VisualFamily, VisualDefinition[]>();
  for (const visual of VISUALIZATION_CATALOG) {
    if (!out.has(visual.family)) out.set(visual.family, []);
    out.get(visual.family)!.push(visual);
  }
  return out;
}

export function findVisual(code: string): VisualDefinition | undefined {
  return VISUALIZATION_CATALOG.find((v2) => v2.code === code);
}
