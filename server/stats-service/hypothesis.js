...
exports.oneSampleTTest = oneSampleTTest;
exports.independentTTest = independentTTest;
exports.pairedTTest = pairedTTest;
exports.oneWayAnova = oneWayAnova;
exports.twoWayAnova = twoWayAnova;
exports.rank = rank;
exports.mannWhitneyU = mannWhitneyU;
exports.wilcoxonSignedRank = wilcoxonSignedRank;
exports.kruskalWallis = kruskalWallis;
exports.chiSquareIndependence = chiSquareIndependence;
exports.chiSquareGoodnessOfFit = chiSquareGoodnessOfFit;
/**
...
function shapiroWilk(values) {
    ...
    if (n > 5) {
        ...
        a[n - 2] = an1;
        a[1] = -an1;
        const phi = (mSquaredSum - 2 * m[n - 1] ** 2 - 2 * m[n - 2] ** 2) /
            (1 - 2 * an ** 2 - 2 * an1 ** 2);
    ...
    const xMean = (0, descriptive_ts_1.mean)(x);
    ...
    if (n === 3) {
        ...
        pValue = Math.max(0, Math.min(1, 1 - (0, distributions_ts_1.normalCdf)((w - mu) / sigma)));
    }
    return {
        nameKey: 'assumption.normality',
        passed: pValue >= 0.05,
...
function levene(groups) {
    ...
    const z = groups.map((g) => {
        const med = (0, descriptive_ts_1.quantile)(g, 0.5);
        return g.map((v) => Math.abs(v - med));
    });
    const zGroupMeans = z.map(descriptive_ts_1.mean);
    const zGrandMean = (0, descriptive_ts_1.mean)(z.flat());
    const numerator = (N - k) * z.reduce((acc, g, i) => acc + g.length * (zGroupMeans[i] - zGrandMean) ** 2, 0);
    const denominator = (k - 1) * z.reduce((acc, g, i) => acc + g.reduce((s, v) => s + (v - zGroupMeans[i]) ** 2, 0), 0);
    const W = denominator === 0 ? 0 : numerator / denominator;
    const pValue = (0, distributions_ts_1.fTestPValue)(W, k - 1, N - k);
    return {
...
function oneSampleTTest(values, mu, alpha = 0.05, tails = 2) {
    const n = values.length;
    ...
    const t = (m - mu) / se;
    const df = n - 1;
    const p = (0, distributions_ts_1.tTestPValue)(t, df, tails);
    const critical = (0, distributions_ts_1.studentTQuantile)(1 - alpha / tails, df);
    const d = (m - mu) / s;
    const normality = shapiroWilk(values);
    const warnings = [];
    if (!normality.passed)
        warnings.push('warning.normality_violated');
    if (n < 30)
...
function independentTTest(groupA, groupB, alpha = 0.05, tails = 2, options = {}) {
    const n1 = groupA.length;
    const n2 = groupB.length;
    const m1 = (0, descriptive_ts_1.mean)(groupA);
    const m2 = (0, descriptive_ts_1.mean)(groupB);
    const v1 = (0, descriptive_ts_1.variance)(groupA);
    const v2 = (0, descriptive_ts_1.variance)(groupB);
    ...
    const warnings = [];
    if (!normalityA.passed || !normalityB.passed)
        warnings.push('warning.normality_violated');
    if (!homogeneity.passed)
        warnings.push('warning.homogeneity_violated_welch_applied');
...
function pairedTTest(before, after, alpha = 0.05, tails = 2) {
    ...
    return {
        ...
        interpretation: {
            ...
            params: {
                meanDifference: Number((0, descriptive_ts_1.mean)(differences).toFixed(4)),
                pValue: result.pValue,
                alpha,
            },
        },
        groups: [
            { label: 'before', n: before.length, mean: (0, descriptive_ts_1.mean)(before), stdDev: (0, descriptive_ts_1.stdDev)(before) },
            { label: 'after', n: after.length, mean: (0, descriptive_ts_1.mean)(after), stdDev: (0, descriptive_ts_1.stdDev)(after) },
        ],
    };
}
function oneWayAnova(labelled, alpha = 0.05) {
    const k = labelled.length;
    const all = labelled.flatMap((g) => g.values);
    const N = all.length;
    const grandMean = (0, descriptive_ts_1.mean)(all);
    const ssBetween = labelled.reduce((acc, g) => acc + g.values.length * ((0, descriptive_ts_1.mean)(g.values) - grandMean) ** 2, 0);
    const ssWithin = labelled.reduce((acc, g) => {
        const m = (0, descriptive_ts_1.mean)(g.values);
    ...
    const p = (0, distributions_ts_1.fTestPValue)(F, dfBetween, dfWithin);
    ...
    const warnings = [];
    if (!homogeneity.passed)
        warnings.push('warning.homogeneity_violated');
    if (normality.some((a) => !a.passed))
        warnings.push('warning.normality_violated');
    if (warnings.length > 0)
        warnings.push('warning.consider_kruskal_wallis');
    // Uji lanjut Tukey HSD — hanya bermakna bila uji utama signifikan.
    const postHoc = [];
    if (p < alpha) {
        const q = (0, distributions_ts_1.tukeyCriticalQ)(k, dfWithin);
        for (let i = 0; i < k; i++) {
            for (let j = i + 1; j < k; j++) {
                const a = labelled[i];
                const b = labelled[j];
                const hsd = q * Math.sqrt((msWithin / 2) * (1 / a.values.length + 1 / b.values.length));
                const difference = (0, descriptive_ts_1.mean)(a.values) - (0, descriptive_ts_1.mean)(b.values);
                postHoc.push({
                    groupA: a.label,
                    groupB: b.label,
    ...
    return {
        ...
        groups: labelled.map((g) => ({
            label: g.label,
            n: g.values.length,
            mean: (0, descriptive_ts_1.mean)(g.values),
            stdDev: (0, descriptive_ts_1.stdDev)(g.values),
        })),
        postHoc,
...
function twoWayAnova(rows, alpha = 0.05) {
    ...
    const msWithin = ssWithin / dfWithin;
    const build = (key, ss, df) => {
        const f = msWithin === 0 || df === 0 ? 0 : ss / df / msWithin;
        const p = (0, distributions_ts_1.fTestPValue)(f, df, dfWithin);
        return {
            key,
    ...
    const warnings = [];
    if (dfWithin <= 0)
        warnings.push('warning.insufficient_replication');
    return {
...
function rank(values) {
    ...
    while (position < indexed.length) {
        ...
        for (let i = position; i <= end; i++)
            ranks[indexed[i].i] = averageRank;
        position = end + 1;
    }
    return ranks;
}
function mannWhitneyU(groupA, groupB, alpha = 0.05) {
    const n1 = groupA.length;
    const n2 = groupB.length;
    ...
    const p = Math.min(1, 2 * (1 - (0, distributions_ts_1.normalCdf)(Math.abs(z))));
    ...
    return {
        ...
        causalityNoteKey: CAUSALITY_ASSOCIATION,
        groups: [
            { label: 'A', n: n1, mean: (0, descriptive_ts_1.mean)(groupA), stdDev: (0, descriptive_ts_1.stdDev)(groupA) },
            { label: 'B', n: n2, mean: (0, descriptive_ts_1.mean)(groupB), stdDev: (0, descriptive_ts_1.stdDev)(groupB) },
        ],
    };
}
function wilcoxonSignedRank(before, after, alpha = 0.05) {
...
function kruskalWallis(labelled, alpha = 0.05) {
    const all = labelled.flatMap((g) => g.values);
    const N = all.length;
    ...
    const p = (0, distributions_ts_1.chiSquarePValue)(H, df);
...
function chiSquareIndependence(table, rowLabels, columnLabels, alpha = 0.05) {
    const rowTotals = table.map((row) => row.reduce((a, b) => a + b, 0));
    const columnCount = table[0]?.length ?? 0;
    const columnTotals = Array.from({ length: columnCount }, (_, j) => table.reduce((acc, row) => acc + (row[j] ?? 0), 0));
    const grandTotal = rowTotals.reduce((a, b) => a + b, 0);
    const expected = table.map((_, i) => Array.from({ length: columnCount }, (_, j) => (rowTotals[i] * columnTotals[j]) / grandTotal));
    let chi = 0;
    ...
    const p = (0, distributions_ts_1.chiSquarePValue)(chi, df);
...
function chiSquareGoodnessOfFit(observed, expectedProportions, labels, alpha = 0.05) {
