...
exports.RECOVERY_CODE_COUNT = exports.TOTP_WINDOW_STEPS = exports.TOTP_DIGITS = exports.TOTP_STEP_SECONDS = void 0;
...
exports.normaliseRecoveryCode = normaliseRecoveryCode;
/**
...
function base32Decode(input) {
    ...
    if (normalised === '')
        throw new Error('Rahasia base32 kosong');
    let bits = 0;
    let value = 0;
    const bytes = [];
    for (const char of normalised) {
        ...
        if (index === -1)
            throw new Error(`Karakter base32 tidak sah: ${char}`);
        value = (value << 5) | index;
        bits += 5;
        if (bits >= 8) {
...
function generateMfaSecret() {
    return base32Encode((0, node_crypto_1.randomBytes)(SECRET_BYTES));
}
/** Nomor langkah waktu untuk sebuah waktu. Diekspor agar dapat dipakai anti-replay. */
function totpCounter(atMs = Date.now()) {
    return Math.floor(atMs / 1000 / exports.TOTP_STEP_SECONDS);
}
/**
...
function hotp(secret, counter) {
    ...
    const digest = (0, node_crypto_1.createHmac)('sha1', secret).update(buffer).digest();
...
function totpCode(secretBase32, counter = totpCounter()) {
...
function verifyTotp(secretBase32, code, options = {}) {
    ...
    if (!new RegExp(`^\\d{${exports.TOTP_DIGITS}}$`).test(candidate))
        return { valid: false, counter: null };
    const secret = base32Decode(secretBase32);
    const centre = totpCounter(options.atMs ?? Date.now());
    const window = options.windowSteps ?? exports.TOTP_WINDOW_STEPS;
    for (let offset = -window; offset <= window; offset++) {
        const counter = centre + offset;
        if (counter < 0)
            continue;
        if (options.minCounter !== undefined && counter <= options.minCounter)
            continue;
...
function otpauthUri(input) {
    ...
    const params = new URLSearchParams({
        ...
        period: String(exports.TOTP_STEP_SECONDS),
    });
...
exports.RECOVERY_CODE_COUNT = 10;
 ...
 * dari kertas (I/1, O/0).
 */
function generateRecoveryCodes(count = exports.RECOVERY_CODE_COUNT) {
    const codes = [];
    for (let i = 0; i < count; i++) {
        ...
        for (let g = 0; g < RECOVERY_GROUPS; g++) {
            ...
            for (let c = 0; c < RECOVERY_GROUP; c++) {
                group += RECOVERY_ALPHABET[(0, node_crypto_1.randomInt)(0, RECOVERY_ALPHABET.length)];
            }
            groups.push(group);
        }
