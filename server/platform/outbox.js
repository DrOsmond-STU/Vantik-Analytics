...
exports.NotificationOutbox = void 0;
/**
 * Outbox notifikasi.
 *
...
class NotificationOutbox {
    ...
    enqueue(input) {
        ...
        this.db
         ...
         VALUES (?,?,?,?,?,?,?,'queued',0,NULL,?,?,NULL)`)
            .run(id, input.tenantId, input.purpose, input.channel, input.recipient, input.subject, input.body, input.sensitive ? 1 : 0, (0, db_ts_1.nowIso)());
     ...
     */
    recordFailure(id, reason, maxAttempts) {
        this.db
            .prepare(`UPDATE notification_outbox
        ...
        const row = this.db.prepare('SELECT status FROM notification_outbox WHERE id = ?').get(id);
    ...
    pendingAll(limit = 50) {
        return this.db
            .prepare(`SELECT id, tenant_id, purpose, channel, recipient, subject, body, status, attempts,
                failure_reason, sensitive, created_at, sent_at
    ...
    pending(tenantId, limit = 100) {
        return this.db
            .prepare(`SELECT id, purpose, channel, recipient, subject, body, status, attempts, failure_reason,
    ...
    list(tenantId, limit = 200) {
        return this.db
            .prepare(`SELECT id, purpose, channel, recipient, subject, status, attempts, failure_reason,
                sensitive, created_at, sent_at
           FROM notification_outbox
          WHERE tenant_id = ?
