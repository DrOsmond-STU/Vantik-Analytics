...
exports.retentionReport = retentionReport;
/**
 * Angka bawaan.
 *
...
function resolveRetention(env = process.env) {
    const out = { ...exports.DEFAULT_RETENTION };
 ...
 * tumbuh selamanya.
 */
function pruneExpiredRows(db, options = {}) {
    const w = options.windows ?? resolveRetention();
    const now = options.now ?? Date.now();
    const deleted = {};
    const skipped = {};
    const run = (table, days, sql, params) => {
        if (days <= 0) {
    ...
    db.transaction(() => {
        ...
        run('active_sessions', w.deadSessions, `DELETE FROM active_sessions
        WHERE (revoked_at IS NOT NULL AND revoked_at < :cutoff)
           OR (revoked_at IS NULL AND expires_at < :cutoff)`, { cutoff: cutoffIso(w.deadSessions, now) });
        run('mfa_challenges', w.mfaChallenges, `DELETE FROM mfa_challenges
        WHERE (consumed_at IS NOT NULL AND consumed_at < :cutoff)
           OR (consumed_at IS NULL AND expires_at < :cutoff)`, { cutoff: cutoffIso(w.mfaChallenges, now) });
        run('password_reset_requests', w.passwordResets, `DELETE FROM password_reset_requests
        WHERE (consumed_at IS NOT NULL AND consumed_at < :cutoff)
           OR (consumed_at IS NULL AND expires_at < :cutoff)`, { cutoff: cutoffIso(w.passwordResets, now) });
        ...
        run('notification_outbox', w.sentNotifications, "DELETE FROM notification_outbox WHERE status = 'sent' AND created_at < :cutoff", { cutoff: cutoffIso(w.sentNotifications, now) });
 ...
 * disk hosting penuh.
 */
function retentionReport(db) {
    const count = (table) => db.prepare(`SELECT COUNT(*) AS n FROM ${table}`).get().n;
    const oldest = (table, column) => db.prepare(`SELECT MIN(${column}) AS m FROM ${table}`).get().m;
        ...
        appendOnly: [
            {
                ...
                note: 'Kekal karena trigger (SECURITY.md Bagian 9). Tidak dapat dipangkas tanpa melepas ' +
            ...
            },
            {
                ...
                note: 'Append-only karena menjadi bukti perhitungan tagihan (SECURITY.md 16.3). ' +
