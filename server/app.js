...
const index_ts_1 = require("./audit-service/index.js");
const db_ts_1 = require("./platform/db.js");
const outbox_ts_1 = require("./platform/outbox.js");
const outboxDispatcher_ts_1 = require("./platform/outboxDispatcher.js");
const scheduler_ts_1 = require("./platform/scheduler.js");
const retention_ts_1 = require("./platform/retention.js");
const transports_ts_1 = require("./platform/transports.js");
const crypto_ts_1 = require("./platform/crypto.js");
const errors_ts_1 = require("./platform/errors.js");
const http_ts_1 = require("./platform/http.js");
const featureFlags_ts_1 = require("./platform/featureFlags.js");
const rbac_ts_1 = require("./platform/rbac.js");
const index_ts_2 = require("./identity-service/index.js");
const index_ts_3 = require("./tenant-service/index.js");
const index_ts_4 = require("./billing-service/index.js");
const index_ts_5 = require("./metering-service/index.js");
const index_ts_6 = require("./data-platform-service/index.js");
const index_ts_7 = require("./stats-service/index.js");
const index_ts_8 = require("./ai-engine-service/index.js");
const index_ts_9 = require("./designer-service/index.js");
...
function createApp(options = {}) {
    ...
    const audit = new index_ts_1.AuditService(db);
    const outbox = new outbox_ts_1.NotificationOutbox(db);
    /**
     ...
     */
    const notificationTransport = (0, transports_ts_1.transportFromEnv)() ?? new index_ts_10.QueueOnlyTransport();
    /**
     * Pengirim antrean.
     *
     ...
     */
    const outboxDispatcher = new outboxDispatcher_ts_1.OutboxDispatcher(outbox, notificationTransport);
    outboxDispatcher.attach();
    const auth = new index_ts_2.AuthService(db, audit, outbox);
    const tenants = new index_ts_3.TenantService(db, audit);
    tenants.seedPlans();
    const deps = { db, audit, auth };
    const app = (0, express_1.default)();
    app.set('trust proxy', true);
    app.use((0, http_ts_1.securityHeaders)());
     ...
     */
    const issueSessionCookie = (res, token, expiresAt) => {
    ...
    const loginLimiter = new http_ts_1.RateLimiter(10, 60_000, db);
    /**
     ...
     */
    const signupLimiter = new http_ts_1.RateLimiter(3, 60 * 60_000, db);
    /**
     ...
     */
    const selfSignupEnabled = () => (process.env.VANTIK_SELF_SIGNUP ?? 'on').toLowerCase() !== 'off';
    const apiLimiter = new http_ts_1.RateLimiter(600, 60_000, db);
    const embedLimiter = new http_ts_1.RateLimiter(120, 60_000, db);
    const idempotency = new http_ts_1.IdempotencyStore(db);
    ...
    app.post('/api/v1/auth/login', loginLimiter.middleware((req) => `login:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (0, http_ts_1.asyncRoute)((req, res) => {
        const body = req.body;
        if (!body.email || !body.password || !body.fingerprint) {
            throw new errors_ts_1.ValidationError('error.missing_credentials');
        }
     ...
     */
    app.post('/api/v1/auth/mfa/verify', loginLimiter.middleware((req) => `mfa:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (0, http_ts_1.asyncRoute)((req, res) => {
        const body = req.body;
     ...
     */
    app.post('/api/v1/devices/transfers', loginLimiter.middleware((req) => `transfer:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (0, http_ts_1.asyncRoute)((req, res) => {
        const body = req.body;
        if (!body.email || !body.password || !body.fingerprint) {
            throw new errors_ts_1.ValidationError('error.missing_credentials');
        }
    ...
    app.post('/api/v1/devices/transfers/:id/verify-otp', loginLimiter.middleware((req) => `transfer-otp:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (0, http_ts_1.asyncRoute)((req, res) => {
    ...
    const embedRenderer = new index_ts_9.EmbedRenderer(db, audit);
    app.get('/embed/v1/dashboard', embedLimiter.middleware((req) => `embed:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (0, http_ts_1.asyncRoute)((req, res) => {
        const token = String(req.query.token ?? '');
        const origin = req.headers.origin ?? req.headers.referer ?? null;
        const normalisedOrigin = origin ? new URL(origin).origin : null;
        const result = embedRenderer.render({ token, origin: normalisedOrigin, ip: (0, http_ts_1.clientIp)(req) });
        if (!result.ok) {
    ...
    api.use((0, http_ts_1.authenticate)(deps));
    api.use(apiLimiter.middleware((req) => `api:${req.ctx?.actor.userId ?? (0, http_ts_1.clientIp)(req)}`));
     ...
     */
    api.post('/me/password', loginLimiter.middleware((req) => `password:${req.ctx?.actor.userId ?? (0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        const body = req.body;
    ...
    api.post('/mfa/activate', loginLimiter.middleware((req) => `mfa-activate:${req.ctx?.actor.userId ?? (0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
    ...
    api.post('/mfa/disable', loginLimiter.middleware((req) => `mfa-disable:${req.ctx?.actor.userId ?? (0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        ...
        if ((0, rbac_ts_1.requiresMfa)(ctx.actor.roleCodes)) {
            res.status(403).json({
    ...
    api.post('/mfa/recovery-codes', loginLimiter.middleware((req) => `mfa-recovery:${req.ctx?.actor.userId ?? (0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
    ...
    api.post('/auth/reauthenticate', loginLimiter.middleware((req) => `reauth:${req.ctx?.actor.userId ?? (0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
    ...
    api.get('/modules', (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        res.json({
            modules: featureFlags_ts_1.MODULE_KEYS.map((key) => ({ key, enabled: ctx.flags.isEnabled(key) })),
            plan: ctx.flags.planCode,
            readOnly: ctx.flags.readOnly,
        });
    });
    /* --- Dataset & Data Quality (PRD 6.11, 6.14) --- */
    const datasetsOf = (req) => new index_ts_6.DatasetService((0, http_ts_1.requireContext)(req), new index_ts_5.MeteringService((0, http_ts_1.requireContext)(req)));
    api.get('/datasets', (req, res) => {
        res.json({ datasets: datasetsOf(req).list(req.query) });
    });
    ...
    api.post('/datasets/upload', (req, res) => {
        ...
        if (!body.filename || !body.contentBase64)
            throw new errors_ts_1.ValidationError('error.upload_payload_required');
        res.status(201).json(datasetsOf(req).upload({
    ...
    api.get('/datasets/:id/rows', (req, res) => {
        res.json(datasetsOf(req).rows(req.params.id, {
            limit: req.query.limit ? Number(req.query.limit) : undefined,
            offset: req.query.offset ? Number(req.query.offset) : undefined,
        }));
    });
    api.post('/datasets/:id/columns/confirm', (req, res) => {
        datasetsOf(req).confirmColumnTypes(req.params.id, req.body.types);
        res.json({ ok: true });
    });
    api.post('/datasets/:id/map', (req, res) => {
        datasetsOf(req).mapColumns(req.params.id, req.body.mappings);
        res.json({ ok: true });
    });
    ...
    const connectionsOf = (req) => new index_ts_6.ConnectionService((0, http_ts_1.requireContext)(req), keyring, undefined, new index_ts_5.MeteringService((0, http_ts_1.requireContext)(req)));
    api.get('/connections', (req, res) => {
        res.json({ connections: connectionsOf(req).list() });
    });
    api.post('/connections', (req, res) => {
        res.status(201).json(connectionsOf(req).create(req.body));
    });
    api.post('/connections/:id/test', (0, http_ts_1.asyncRoute)(async (req, res) => {
        res.json(await connectionsOf(req).testConnection(req.params.id));
    }));
    api.post('/connections/:id/rotate', (req, res) => {
        connectionsOf(req).rotateSecrets(req.params.id, req.body.secrets);
        res.json({ ok: true });
    });
    ...
    const modelingOf = (req) => new index_ts_6.DataModelingService((0, http_ts_1.requireContext)(req));
    api.get('/model/tables', (req, res) => {
        res.json({ tables: modelingOf(req).listTables() });
    });
    api.post('/model/tables', (req, res) => {
        res.status(201).json(modelingOf(req).createTable(req.body));
    });
    api.post('/model/fields', (req, res) => {
        res.status(201).json(modelingOf(req).addField(req.body));
    });
    ...
    api.get('/model/dictionary', (req, res) => {
        res.json({ terms: modelingOf(req).listDictionary(req.query.q) });
    });
    api.post('/model/dictionary', (req, res) => {
        modelingOf(req).upsertTerm(req.body);
        res.json({ ok: true });
    });
    /* --- KPI Center (PRD 6.15) --- */
    const kpiOf = (req) => new index_ts_6.KpiService((0, http_ts_1.requireContext)(req));
    api.get('/kpis', (req, res) => {
        res.json({ kpis: kpiOf(req).list() });
    });
    api.post('/kpis', (req, res) => {
        res.status(201).json(kpiOf(req).create(req.body));
    });
    ...
    api.post('/kpis/:id/thresholds', (req, res) => {
        kpiOf(req).setThresholds(req.params.id, req.body.thresholds);
        res.json({ ok: true });
    });
    api.post('/kpis/:id/propose', (req, res) => {
        res.json({ approvalId: kpiOf(req).proposeChange(req.params.id, req.body) });
    });
    ...
    api.post('/kpis/:id/capture', (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        const body = req.body;
        const rows = new index_ts_6.DatasetService(ctx).allRows(body.datasetId);
        res.json({ captured: kpiOf(req).captureScores(req.params.id, body.period, rows) });
    });
    /* --- Alert Center (PRD 6.16) --- */
    const alertsOf = (req) => new index_ts_10.AlertService((0, http_ts_1.requireContext)(req), notificationTransport, outbox);
    api.get('/alerts/rules', (req, res) => {
        res.json({ rules: alertsOf(req).listRules() });
    });
    api.post('/alerts/rules', (req, res) => {
        res.status(201).json(alertsOf(req).createRule(req.body));
    });
    api.post('/alerts/rules/:id/toggle', (req, res) => {
        alertsOf(req).toggleRule(req.params.id, Boolean(req.body.enabled));
        res.json({ ok: true });
    });
    ...
    api.post('/alerts/events/:id/acknowledge', (req, res) => {
        alertsOf(req).acknowledge(req.params.id, req.body.note);
        res.json({ ok: true });
    });
    api.get('/alerts/latency', (req, res) => {
        res.json(alertsOf(req).detectionToNotificationStats());
    });
    api.post('/alerts/evaluate', (0, http_ts_1.asyncRoute)(async (req, res) => {
        res.json({ triggered: await alertsOf(req).evaluate(req.body) });
    }));
    /* --- Dashboard / Report / Visualization / Embed (Domain 2) --- */
    const dashboardsOf = (req) => new index_ts_9.DashboardService((0, http_ts_1.requireContext)(req), new index_ts_5.MeteringService((0, http_ts_1.requireContext)(req)));
    const reportsOf = (req) => new index_ts_9.ReportService((0, http_ts_1.requireContext)(req));
    const embedOf = (req) => new index_ts_9.EmbedService((0, http_ts_1.requireContext)(req), new index_ts_5.MeteringService((0, http_ts_1.requireContext)(req)));
    ...
    api.post('/dashboards', (req, res) => {
        res.status(201).json(dashboardsOf(req).create(req.body));
    });
    api.get('/dashboards/:id', (req, res) => {
        res.json(dashboardsOf(req).get(req.params.id));
    });
    api.put('/dashboards/:id/draft', (req, res) => {
        dashboardsOf(req).saveDraft(req.params.id, req.body.widgets);
        res.json({ ok: true });
    });
    ...
    api.post('/reports', (req, res) => {
        res.status(201).json(reportsOf(req).create(req.body));
    });
    api.put('/reports/:id/blocks', (req, res) => {
        reportsOf(req).saveBlocks(req.params.id, req.body.blocks);
        res.json({ ok: true });
    });
    api.post('/reports/:id/signature', (req, res) => {
        reportsOf(req).setSignature(req.params.id, req.body);
        res.json({ ok: true });
    });
    ...
    api.post('/dashboards/:id/embed-tokens', (req, res) => {
        res.status(201).json(embedOf(req).issue({ ...req.body, dashboardId: req.params.id }));
    });
    ...
    api.post('/ai/ask', (0, http_ts_1.asyncRoute)(async (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        const service = new index_ts_8.AiAnalyticsService(ctx, undefined, new index_ts_5.MeteringService(ctx));
        const body = req.body;
        res.json(await service.ask(body.question, { datasetId: body.datasetId, locale: body.locale }));
    }));
    api.get('/ai/history', (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        res.json({ queries: new index_ts_8.AiAnalyticsService(ctx).history() });
    });
    api.post('/forecast', (req, res) => {
        res.json(new index_ts_8.ForecastService((0, http_ts_1.requireContext)(req)).run(req.body));
    });
    api.post('/rca/generate', (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        const body = req.body;
        const rows = new index_ts_6.DatasetService(ctx).allRows(body.datasetId);
        res.json(new index_ts_8.RcaService(ctx).generate({ ...body, rows }));
    });
    api.post('/rca/:id/validate', (req, res) => {
        new index_ts_8.RcaService((0, http_ts_1.requireContext)(req)).validate(req.params.id, req.body);
        res.json({ ok: true });
    });
    api.get('/rca', (req, res) => {
        res.json({ records: new index_ts_8.RcaService((0, http_ts_1.requireContext)(req)).list() });
    });
    api.get('/discovery/:datasetId', (req, res) => {
        res.json(new index_ts_8.DiscoveryService((0, http_ts_1.requireContext)(req)).explore(req.params.datasetId));
    });
    api.post('/narrative/generate', (req, res) => {
        res.json(new index_ts_8.NarrativeService((0, http_ts_1.requireContext)(req)).generate(req.body));
    });
    api.get('/narrative', (req, res) => {
        res.json({ reports: new index_ts_8.NarrativeService((0, http_ts_1.requireContext)(req)).list() });
    });
    /* --- Analisis Statistik (Domain 4) --- */
    const statsOf = (req) => new index_ts_7.StatsService((0, http_ts_1.requireContext)(req));
    /**
     ...
     */
    function statsSpec(body, fields) {
        if (typeof body !== 'object' || body === null) {
            throw new errors_ts_1.ValidationError('error.validation_failed', { reason: 'body' });
        }
        const record = body;
        for (const field of ['datasetId', ...(fields.text ?? [])]) {
            const value = record[field];
            if (typeof value !== 'string' || value.trim() === '') {
                throw new errors_ts_1.ValidationError('error.validation_failed', { field });
            }
        }
    ...
    api.post('/stats/descriptive', (req, res) => {
        res.json(statsOf(req).descriptive(statsSpec(req.body, { list: ['fields'] })));
    });
    api.post('/stats/hypothesis', (req, res) => {
        res.json(statsOf(req).hypothesis(statsSpec(req.body, { text: ['test'] })));
    });
    api.post('/stats/correlation', (req, res) => {
        res.json(statsOf(req).correlation(statsSpec(req.body, { list: ['fields'] })));
    });
    api.post('/stats/regression', (req, res) => {
        res.json(statsOf(req).regression(statsSpec(req.body, {
            text: ['kind', 'responseField'],
            list: ['predictorFields'],
        })));
    });
    /* --- Digital Twin (PRD 6.17) --- */
    const twinOf = (req) => new index_ts_11.DigitalTwinService((0, http_ts_1.requireContext)(req), new index_ts_10.AlertService((0, http_ts_1.requireContext)(req), notificationTransport, outbox));
    api.get('/twin/floor-plan', (req, res) => {
        res.json({ zones: twinOf(req).floorPlan() });
    });
    api.post('/twin/zones', (req, res) => {
        res.status(201).json(twinOf(req).createZone(req.body));
    });
    api.post('/twin/assets', (req, res) => {
        res.status(201).json(twinOf(req).createAsset(req.body));
    });
    api.get('/twin/assets/:id', (req, res) => {
        res.json(twinOf(req).assetDetail(req.params.id));
    });
    api.post('/twin/readings', (0, http_ts_1.asyncRoute)(async (req, res) => {
        res.json(await twinOf(req).ingestReading(req.body));
    }));
    api.get('/twin/maintenance-queue', (req, res) => {
        res.json({ queue: twinOf(req).maintenanceQueue() });
    });
    api.post('/twin/assets/:id/tickets', (req, res) => {
        res.status(201).json(twinOf(req).createTicket(req.params.id, req.body));
    });
    api.post('/twin/simulate', (req, res) => {
        res.json(twinOf(req).simulate(req.body));
    });
    api.get('/twin/energy', (req, res) => {
        res.json({ comparison: twinOf(req).energyComparison(req.query.sensor) });
    });
    ...
    api.get('/cockpit/executive', (req, res) => {
        ...
        res.json(new index_ts_12.CockpitService((0, http_ts_1.requireContext)(req)).executive(period));
    });
    api.get('/cockpit/operational', (req, res) => {
        const period = String(req.query.period ?? new Date().toISOString().slice(0, 7));
        res.json(new index_ts_12.CockpitService((0, http_ts_1.requireContext)(req)).operational(period, req.query.division));
    });
    api.get('/scorecard', (req, res) => {
        const period = String(req.query.period ?? new Date().toISOString().slice(0, 7));
        res.json(new index_ts_12.BalancedScorecardService((0, http_ts_1.requireContext)(req)).scorecard(period, {
            comparePeriod: req.query.compare,
        }));
    });
    api.post('/scorecard/seed', (req, res) => {
        new index_ts_12.BalancedScorecardService((0, http_ts_1.requireContext)(req)).seedStandardPerspectives();
        res.json({ ok: true });
    });
    api.post('/scorecard/perspectives', (req, res) => {
        res.status(201).json(new index_ts_12.BalancedScorecardService((0, http_ts_1.requireContext)(req)).addPerspective(req.body));
    });
    api.post('/scorecard/objectives', (req, res) => {
        res.status(201).json(new index_ts_12.BalancedScorecardService((0, http_ts_1.requireContext)(req)).addObjective(req.body));
    });
    api.get('/scorecard/strategy-map', (req, res) => {
        res.json(new index_ts_12.BalancedScorecardService((0, http_ts_1.requireContext)(req)).strategyMap());
    });
    /* --- Administrasi Sistem (Domain 7) --- */
    api.get('/employees', (req, res) => {
        res.json({ employees: new index_ts_2.EmployeeService((0, http_ts_1.requireContext)(req)).list(req.query) });
    });
    api.post('/employees', (req, res) => {
        res.status(201).json(new index_ts_2.EmployeeService((0, http_ts_1.requireContext)(req)).create(req.body));
    });
    api.post('/employees/:id/status', (req, res) => {
        const body = req.body;
        res.json(new index_ts_2.EmployeeService((0, http_ts_1.requireContext)(req)).changeStatus(req.params.id, body.status, auth));
    });
    api.post('/employees/import', (req, res) => {
        res.json(new index_ts_2.EmployeeService((0, http_ts_1.requireContext)(req)).importCsv(req.body.csv));
    });
    api.get('/employees/access-reviews', (req, res) => {
        res.json({ overdue: new index_ts_2.EmployeeService((0, http_ts_1.requireContext)(req)).overdueAccessReviews() });
    });
    api.get('/authorization/users', (req, res) => {
        res.json({ users: new index_ts_2.AuthorizationService((0, http_ts_1.requireContext)(req)).listUsers() });
    });
    api.get('/authorization/roles', (req, res) => {
        res.json({ roles: new index_ts_2.AuthorizationService((0, http_ts_1.requireContext)(req)).listRoles() });
    });
    api.post('/authorization/users', (req, res) => {
        res.status(201).json(new index_ts_2.AuthorizationService((0, http_ts_1.requireContext)(req)).createUser(req.body));
    });
    api.put('/authorization/users/:id/roles', (req, res) => {
        new index_ts_2.AuthorizationService((0, http_ts_1.requireContext)(req)).setRoles(req.params.id, req.body.roles);
        res.json({ ok: true });
    });
    api.put('/authorization/rls', (req, res) => {
        const body = req.body;
        new index_ts_2.AuthorizationService((0, http_ts_1.requireContext)(req)).setRls(body.subject, body.rules);
        res.json({ ok: true });
    });
    api.post('/authorization/users/:id/disable', (req, res) => {
        new index_ts_2.AuthorizationService((0, http_ts_1.requireContext)(req)).disableUser(req.params.id, auth);
        res.json({ ok: true });
    });
    api.post('/authorization/users/:id/password', (req, res) => {
        ...
        new index_ts_2.AuthorizationService((0, http_ts_1.requireContext)(req)).resetPassword(req.params.id, next, auth);
        res.json({ ok: true });
    });
    api.get('/authorization/access-review', (req, res) => {
        res.json(new index_ts_2.AuthorizationService((0, http_ts_1.requireContext)(req)).accessReview());
    });
    api.get('/audit', (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        ctx.require('audit:read', { module: 'Log Aktivitas' });
        res.json(audit.query(ctx.tenant.id, {
    ...
    api.get('/audit/export', (req, res) => {
        ...
        res.type('text/csv').send(audit.exportCsv(ctx.tenant.id, req.query));
    });
    api.get('/devices/mine', (req, res) => {
        res.json({ devices: new index_ts_2.DeviceService((0, http_ts_1.requireContext)(req)).myDevices() });
    });
    api.get('/devices', (req, res) => {
        res.json({ devices: new index_ts_2.DeviceService((0, http_ts_1.requireContext)(req)).listAll() });
    });
    api.post('/devices/:id/unbind', (req, res) => {
        new index_ts_2.DeviceService((0, http_ts_1.requireContext)(req)).unbind(req.params.id, String(req.body.reason ?? ''), auth);
        res.json({ ok: true });
    });
    api.get('/devices/transfers', (req, res) => {
        res.json({ requests: new index_ts_2.DeviceService((0, http_ts_1.requireContext)(req)).pendingTransfers() });
    });
    api.post('/devices/transfers/:id/approve', (req, res) => {
        new index_ts_2.DeviceService((0, http_ts_1.requireContext)(req)).approveTransfer(req.params.id, auth);
        res.json({ ok: true });
    });
    /**
    ...
    api.get('/sessions', (req, res) => {
        res.json({ sessions: new index_ts_2.DeviceService((0, http_ts_1.requireContext)(req)).activeSessions() });
    });
    /* --- Langganan & Billing (Domain 8) --- */
    const billingOf = (req) => new index_ts_4.BillingService((0, http_ts_1.requireContext)(req), new index_ts_5.MeteringService((0, http_ts_1.requireContext)(req)), outbox);
    api.get('/subscription', (req, res) => {
        res.json({
            subscription: billingOf(req).currentPlan(),
            plans: billingOf(req).availablePlans(),
            cycles: featureFlags_ts_1.BILLING_CYCLES,
        });
    });
    api.post('/subscription/preview', (req, res) => {
        res.json(billingOf(req).previewPlanChange(req.body.plan));
    });
    ...
    api.post('/subscription/cancel', (req, res) => {
        res.json(billingOf(req).cancel(req.body.reason));
    });
    /**
     * Perpanjangan.
     *
    ...
    api.get('/usage', (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        const metering = new index_ts_5.MeteringService(ctx);
        res.json({
    ...
    api.get('/tenants', (req, res) => {
        res.json({ tenants: tenants.listTenants((0, http_ts_1.requireContext)(req)) });
    });
    api.patch('/tenant/branding', (req, res) => {
        tenants.configureBranding((0, http_ts_1.requireContext)(req), req.body);
        res.json({ ok: true });
    });
    api.post('/tenants/:id/suspend', (req, res) => {
        tenants.suspend((0, http_ts_1.requireContext)(req), req.params.id, String(req.body.reason ?? ''));
        res.json({ ok: true });
    });
    /* --- Persetujuan pendaftaran mandiri --- */
    api.get('/tenants/pending', (req, res) => {
        res.json({ registrations: tenants.listPendingRegistrations((0, http_ts_1.requireContext)(req)) });
    });
    /**
    ...
    api.post('/tenants/:id/restore', (req, res) => {
        tenants.restore((0, http_ts_1.requireContext)(req), req.params.id);
        res.json({ ok: true });
    });
    api.get('/tenant/export', (req, res) => {
        res.json(tenants.exportTenantData((0, http_ts_1.requireContext)(req)));
    });
    api.get('/tenant/operator-access', (req, res) => {
        res.json({ trail: tenants.operatorAccessTrail((0, http_ts_1.requireContext)(req)) });
    });
     ...
     */
    app.post('/api/v1/system/scheduler/run', loginLimiter.middleware((req) => `scheduler:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (0, http_ts_1.asyncRoute)(async (req, res) => {
        ...
        if (!(0, node_crypto_1.timingSafeEqual)(Buffer.from(provided, 'utf8'), Buffer.from(expected, 'utf8'))) {
            res.status(401).json({ error: { key: 'error.unauthenticated', detail: null } });
            return;
        }
    ...
    app.get('/api/v1/public/plans', (_req, res) => {
        res.json({
            plans: featureFlags_ts_1.PLAN_CATALOG.map((plan) => ({
                ...
                prices: (0, featureFlags_ts_1.planPrices)(plan),
     ...
     */
    app.post('/api/v1/public/signup', signupLimiter.middleware((req) => `signup:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (0, http_ts_1.asyncRoute)((req, res) => {
        ...
        if (!body.organisationName || !body.slug || !body.planCode || !body.fullName || !body.email || !body.password) {
            throw new errors_ts_1.ValidationError('error.signup_incomplete');
        }
        ...
        if (body.billingCycle !== undefined && !(0, featureFlags_ts_1.isBillingCycle)(body.billingCycle)) {
            throw new errors_ts_1.ValidationError('error.billing_cycle_unknown', { cycle: body.billingCycle });
        }
        ...
        }, `signup:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`);
        ...
        for (const recipient of tenants.operatorContacts()) {
            outbox.enqueue({
                ...
                body: `Organisasi "${String(body.organisationName).slice(0, 120)}" mendaftar dengan alamat ` +
     ...
     */
    app.post('/api/v1/auth/password-reset/request', loginLimiter.middleware((req) => `reset-req:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (0, http_ts_1.asyncRoute)((req, res) => {
        const body = req.body;
        if (!body.email)
            throw new errors_ts_1.ValidationError('error.email_required');
    ...
    app.post('/api/v1/auth/password-reset/confirm', loginLimiter.middleware((req) => `reset-confirm:${(0, http_ts_1.clientIp)(req) ?? 'unknown'}`), (0, http_ts_1.asyncRoute)((req, res) => {
        const body = req.body;
    ...
    app.post('/webhooks/payment', (0, http_ts_1.authenticate)(deps), (0, http_ts_1.asyncRoute)((req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        const billing = new index_ts_4.BillingService(ctx, new index_ts_5.MeteringService(ctx));
        const secret = process.env.VANTIK_PAYMENT_WEBHOOK_SECRET ?? '';
        const result = billing.handleGatewayWebhook(req.rawBodyText ?? '', String(req.headers['x-signature'] ?? ''), secret);
     ...
     */
    const sweepAlerts = async (ctx) => {
        const period = new Date().toISOString().slice(0, 7);
        const breaching = new index_ts_6.KpiService(ctx).breaching(period);
        if (breaching.length === 0)
            return 0;
        const alerts = new index_ts_10.AlertService(ctx, notificationTransport, outbox);
        let triggered = 0;
     ...
     */
    const dispatchScheduledReports = (ctx) => {
        const due = ctx.db.all('reports');
        let queued = 0;
        for (const report of due) {
            if (!report.schedule_cron || !report.schedule_recipients_json)
                continue;
            const recipients = JSON.parse(report.schedule_recipients_json);
     ...
     */
    const enforceSubscriptions = (ctx) => new index_ts_4.BillingService(ctx, new index_ts_5.MeteringService(ctx), outbox).enforceLifecycle().actions;
    const scheduler = new scheduler_ts_1.Scheduler(db, audit, {
         ...
         */
        'retention.prune': (database) => (0, retention_ts_1.pruneExpiredRows)(database).total,
        /**
    ...
    api.get('/system/scheduler', (req, res) => {
        ...
        res.json({ jobs: scheduler.status(), intervalMs: scheduler_ts_1.SCHEDULER_INTERVAL_MS });
    });
    /**
    ...
    api.get('/system/notification-channels', (req, res) => {
        const ctx = (0, http_ts_1.requireContext)(req);
        ctx.require('device:read', { module: 'Perangkat & Sesi' });
        const routing = notificationTransport;
        res.json({
