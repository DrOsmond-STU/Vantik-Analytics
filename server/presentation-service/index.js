...
exports.BalancedScorecardService = exports.STANDARD_PERSPECTIVES = exports.CockpitService = void 0;
/**
...
class CockpitService {
    ...
    executive(period) {
        ...
        const allKpis = this.ctx.db.all('kpi_definition');
        // Saring KPI yang bersumber dari dataset non-Certified.
        const certified = new Set(this.ctx.db
            .all('dataset_catalog', { certification: 'certified' })
            .map((d) => d.id));
        ...
        const topRisks = kpis
            ...
            const trendKey = !latest || !previous
                ? 'stable'
    ...
    operational(period, division) {
        ...
        const alerts = this.ctx.db
            .all('alert_events', { acknowledged_at: null }, { orderBy: 'detected_at DESC', limit: 20 })
            .map((event) => ({
            id: event.id,
            label: this.ctx.db.get('alert_rules', { id: event.rule_id })?.name ?? '—',
    ...
    buildCockpitKpi(kpi, period, dimensionKey) {
        const scores = this.ctx.db.all('kpi_score_history', { kpi_id: kpi.id }, { orderBy: 'period DESC', limit: 120 });
        // RLS: baris di luar cakupan pengguna tidak pernah masuk hasil.
        const permitted = scores.filter((row) => {
            if (dimensionKey && row.dimension_key !== dimensionKey)
                return false;
            if (this.ctx.rls.isUnrestricted)
                return true;
            if (row.dimension_key === null)
                return false; // agregat lintas dimensi disembunyikan
        ...
        const current = sePeriode.find((s) => s.dimension_key === null) ??
        ...
        const owner = kpi.owner_employee_id
            ? this.ctx.db.get('employee_master', { id: kpi.owner_employee_id })
            : undefined;
        return {
            id: kpi.id,
            name: kpi.name,
...
class BalancedScorecardService {
    ...
    seedStandardPerspectives(level = 'corporate', parentScorecard) {
        ...
        exports.STANDARD_PERSPECTIVES.forEach((perspective, index) => {
    ...
    addObjective(input) {
        this.ctx.require('balanced_scorecard:read', { module: 'Balanced Scorecard' });
        this.ctx.requireWritable();
        if (!this.ctx.db.get('bsc_perspectives', { id: input.perspectiveId }))
            throw new errors_ts_1.NotFoundError();
        if (input.kpiId && !this.ctx.db.get('kpi_definition', { id: input.kpiId })) {
            throw new errors_ts_1.ValidationError('error.kpi_unknown');
        }
        const id = (0, db_ts_1.newId)('bso');
    ...
    scorecard(period, options = {}) {
        this.ctx.require('balanced_scorecard:read', { module: 'Balanced Scorecard' });
        this.ctx.requireModule('balanced_scorecard');
        const level = options.level ?? 'corporate';
        const certified = new Set(this.ctx.db.all('dataset_catalog', { certification: 'certified' }).map((d) => d.id));
        const perspectiveRows = this.ctx.db.all('bsc_perspectives', { scorecard_level: level }, { orderBy: 'sort_order' });
        const perspectives = perspectiveRows.map((p) => {
            const objectiveRows = this.ctx.db.all('bsc_objectives', { perspective_id: p.id });
            const objectives = objectiveRows.map((o) => {
                const kpi = o.kpi_id ? this.ctx.db.get('kpi_definition', { id: o.kpi_id }) : undefined;
                ...
                const score = eligible && kpi
                    ? this.ctx.db.get('kpi_score_history', {
                        kpi_id: kpi.id,
                        period,
                        dimension_key: null,
                    })
                ...
                const owner = o.owner_employee_id
                    ? this.ctx.db.get('employee_master', { id: o.owner_employee_id })
                    : undefined;
                return {
                    id: o.id,
                    name: o.name,
                    ...
                    initiatives: JSON.parse(o.initiatives_json),
                    causes: JSON.parse(o.causes_json),
                };
            });
        ...
        const compositeScore = Number((perspectives.reduce((acc, p) => acc + p.score * p.weight, 0) / totalWeight).toFixed(2));
        let comparison = null;
    ...
    scorecardCompositeOnly(period, level) {
        const perspectiveRows = this.ctx.db.all('bsc_perspectives', {
            scorecard_level: level,
        });
        let weighted = 0;
        let totalWeight = 0;
        for (const p of perspectiveRows) {
            const objectives = this.ctx.db.all('bsc_objectives', { perspective_id: p.id });
            const scores = objectives
                .map((o) => o.kpi_id
                ? this.ctx.db.get('kpi_score_history', { kpi_id: o.kpi_id, period, dimension_key: null })
                : undefined)
                .filter((s) => Boolean(s));
            if (scores.length === 0)
                continue;
     ...
     */
    strategyMap(level = 'corporate') {
        this.ctx.require('balanced_scorecard:read', { module: 'Balanced Scorecard' });
        const card = this.scorecard(new Date().toISOString().slice(0, 7), { level });
        const nodes = card.perspectives.flatMap((p) => p.objectives.map((o) => ({
            id: o.id,
            name: o.name,
            perspective: p.code,
            achievement: o.achievement,
        })));
        const edges = card.perspectives.flatMap((p) => p.objectives.flatMap((o) => o.causes.map((target) => ({ from: o.id, to: target }))));
        return { nodes, edges };
    }
    /**
     ...
     */
    cascade(parentPerspectiveId, divisionName) {
        this.ctx.require('balanced_scorecard:read', { module: 'Balanced Scorecard' });
        this.ctx.requireWritable();
        const parent = this.ctx.db.get('bsc_perspectives', { id: parentPerspectiveId });
        if (!parent)
            throw new errors_ts_1.NotFoundError();
        const id = (0, db_ts_1.newId)('bsp');
        this.ctx.db.insert('bsc_perspectives', {
...
exports.BalancedScorecardService = BalancedScorecardService;
