...
npm test              # 598 test (551 backend + 47 komponen web)
...
| Payment gateway mana | **Xendit** dan **Midtrans** terpasang (QRIS/VA/e-wallet lewat halaman penyedia), dikonfigurasi lewat `.env` dan kosong secara bawaan. Selama kosong, pembayaran dicatat operator platform (`billing:settle`) — **pelanggan tidak dapat menyatakan pembayarannya sendiri**. |
...
| `tests/payment-gateway.test.ts` | Tautan pembayaran dan kabar pembayaran diperiksa terhadap server tiruan berbentuk Xendit & Midtrans: kunci rahasia tidak pernah muncul di alasan kegagalan, token/tanda tangan palsu ditolak, gateway yang belum dikonfigurasi menolak semua kabar, kabar yang **dikirim ulang** tidak memajukan masa berlaku dua kali, `capture` yang masih ditinjau penipuan tidak dianggap lunas, dan gateway yang gagal tidak menggagalkan penerbitan faktur |
